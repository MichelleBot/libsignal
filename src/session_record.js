...
class SessionEntry {
   ...
   deleteChain(key) {
      assertBuffer(key)
      const id = key.toString('base64')
      if (!this._chains.hasOwnProperty(id)) throw new ReferenceError("Not Found")
...
class SessionRecord {
   ...
   getSession(key) {
      ...
      if (session && session.indexInfo.baseKeyType === BaseKeyType.OURS) throw new Error("Tried to lookup a session using our basekey")
      return session
   }
   ...
   closeSession(session) {
      if (this.isClosed(session)) return console.warn("Session already closed", session)
